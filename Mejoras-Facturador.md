...
12. **Pedidos web se facturan "a ciegas"**: la tabla muestra pedido/fecha/cliente/pago/total y el botón "Facturar" directo, pero nunca se ven los ítems del pedido (qué cristales/armazón pidió) antes de emitir la factura fiscal real. Además el motor toma la primera persona del padrón si hay varias con el mismo DNI, sin preguntar (a diferencia de Emitir/Presupuestos, que sí muestran el modal de elegir). Agregar detalle de ítems antes de facturar + el mismo modal de selección.
